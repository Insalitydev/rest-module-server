...
class UserAPI(MethodView):
	def get(self, id=None):
		if id is None:
			return "Success GET Method ex1, list"
		else:
			return "Succes Get Method ex1, id %i" % id
	def post(self):
		keys = ["name"]
		try:
			data = json.loads(request.data.decode("utf-8"))
		except ValueError:
			return "Wrong JSON data ex1"

		for key in keys:
			if not key in data:
				return "Wrong JSON keys ex1"

		return "Success POST Method ex1. User: %s" % data["name"]
